            ...
            <div class="row">
                <div class="col">
                    <a class="btn btn-primary btn-sm"
                       href="{{ route('admin.account.transactions.export', ['account_id' => $account->id, 'month' => $selectedMonth]) }}">{{ trans('global.excel_export') }}</a>
                    @if($account->email)
                        <a class="btn btn-primary btn-sm"
                           href="{{ route('admin.account.transactions.export', ['account_id' => $account->id, 'month' => $selectedMonth, 'email' => 1]) }}">{{ trans('global.send_email') }}</a>
                    @else
                        <div class="row">
                            <div class="col">
